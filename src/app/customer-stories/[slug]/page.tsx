  ...
  return (
    <section
      className="backdrop-blur-[16px] shadow-[inset_0px_4px_10.5px_0px_#EFEFEF40]
        border border-white/10 rounded-[24px] flex flex-col justify-center items-center
         mb-6 md:mb-20 md:px-10 py-3 md:py-10 max-w-5xl"
      style={{
        background:
    ...
    >
      <div className="flex flex-col justify-center items-center gap-5">
        ...
        <div className="flex flex-col justify-center items-center gap-4 md:gap-7">
          ...
          <div className="flex flex-col gap-2 justify-center items-center px-1 lg:px-6">
            <h1 className=" text-xl sm:text-3xl md:text-4xl font-extrabold">
              {story.customerName}
            </h1>
            <p className="text-sm sm:text-base md:text-lg text-gray-300 mt-4 text-center px-2">
              {story.customerDesc}
            </p>
          </div>

          <div className="mx-5 mt-6 mb-6 md:mb-20 flex flex-col md:flex-row gap-4 md:gap-16">
            {story.highlight.map((item, index) => (
              <div
                ...
                rounded-2xl flex flex-col gap-2 justify-center items-center
                 px-2 py-4 md:px-10 md:py-10 w-full md:h-auto md:max-w-80"
                style={{
                  background:
              ...
              >
                <p
                  className="text-xl sm:text-3xl md:text-5xl leading-[120%] font-bold 
                  whitespace-nowrap text-transparent bg-clip-text
                   bg-gradient-to-br from-[#007182] to-[#B0B0B0]"
                >
                  {item.title}
                </p>
                <p className="text-sm p-1 sm:text-lg md:text-xl leading-[120%] font-bold text-white text-center">
                  {item.subtitle}
                </p>
              </div>
            ))}
...
const StrategyAndExecution = ({ strategy }: { strategy: string[] }) => {
  return (
    <section className="flex flex-col justify-start items-start gap-4 md:gap-7 mb-7">
      <h2 className="text-2xl md:text-3xl font-bold text-[#007182BF] text-start ">
        Strategy & Execution
      </h2>
      <div className=" text-[#BBBBBB] space-y-3 px-1 text-xs md:text-base">
...
const Results = ({ results }: { results: string[] }) => {
  return (
    <section className="flex flex-col justify-start items-start gap-4 md:gap-7 mb-7">
      ...
      <div className="text-[#BBBBBB] space-y-3  md:mx-0 px-1 text-xs md:text-base flex flex-col justify-start items-start">
...
export default async function CustomerStoryPage({
  ...
  return (
    <div
     ...
     min-h-screen px-4 sm:px-10 lg:px-8 py-16 text-white w-full mx-auto "
    >
      <div
      ...
      ></div>
      <div className="flex flex-col  lg:items-start px-6">
        <Header story={story} />
